...
Examples:
  ...
  ccr analyze /path/to/repo --file "app.py" --vuln "SQL Injection" --severity "HIGH"
    ...
    parser.add_argument(
        '--finding',
        help='Path to JSON file containing finding details'
    )
    
    parser.add_argument(
        '--file',
        help='File path for inline finding'
    )
    ...
    if args.finding:
        ...
        if not all([args.file, args.vulnerability, args.severity]):
            print("Error: --file, --vuln, and --severity must all be provided together",
                  file=sys.stderr)
            return 1
...
def print_human_readable(result, analyzer, verbose=False):
    ...
    print(f"\n📁 Repository Overview:")
    print(f"   Languages: {', '.join(context['languages']) if context['languages'] else 'Not detected'}")
